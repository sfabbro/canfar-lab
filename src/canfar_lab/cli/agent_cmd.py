    ...
    *,
    ...
    if not rows:
        if kind or agent:
            ...
            ui.print_hint("  canfar agent plugins list")
        else:
...
def agent_setup_cmd(
    ...
    if agent_ids or all_agents or recommended:
        ...
        if agent_ids:
            ui.print_hint("  canfar agent verify        # confirm configs are healthy")
            ui.print_hint("  canfar agent config <id>   # show/edit an agent's config")
        if exit_code:
            raise typer.Exit(exit_code)
        return
    ...
    if result.ok and not result.partial:
        ...
        ui.print_error("Agent setup failed")
    ui.print_hint("  canfar agent install kilo|goose|cline|opencode")
    ui.print_hint("  npx skills add astroai/canfar-skills")
    ui.print_hint("  canfar agent plugins install ray-manager-mcp")
    if result.exit_code:
        raise typer.Exit(result.exit_code)
...
def agent_verify_cmd(
    ...
    if drift:
        ...
        for d in drift:
            ui.print_warn(f"  {d}")
        ui.print_hint("Tip: Run `canfar agent verify --fix` to reconcile.")
    if issues:
        ui.print_error("Agent setup incomplete:\n  " + "\n  ".join(issues))
        ui.print_hint("Tip: Run `canfar agent verify --fix`.")
        raise typer.Exit(1)
...
def _install_one_agent(tool: str, *, dry_run: bool) -> None:
    ...
    if tool in agent_install.TOOLS:
        ...
        raise LabError(f"Unknown tool: {tool}", hint="canfar agent list")
...
def _post_install_hint(tool: str) -> None:
    ...
    if not cfg:
        ui.print_hint(f"  canfar agent setup {tool}   # MCP / plugins")
        return
    path = expand_home(str(cfg), Path.home())
    if path.is_file():
        ui.print_hint(f"  config: {path}")
        if str((agent.get("config") or {}).get("format")) == "markdown":
            ui.print_hint(f"  canfar agent config {tool}   # show notes")
        else:
            ui.print_hint(f"  canfar agent config {tool}")
    else:
        ui.print_hint(f"  canfar agent setup {tool}   # create {path}")
...
def agent_install_cmd(
    ...
    if not names:
        if opts.json:
            ui.print_json(
                {
                    "help": "canfar agent install NAME [NAME…]",
                    "try": ["list"],
                }
            )
            return
        ...
        ui.print_hint("  canfar agent list")
        ui.print_hint("  canfar agent install NAME [NAME…]")
        return
...
def agent_wipe_cmd(
    ...
    if opts.dry_run:
        ...
        return
    ui.print_ok("Agent layer wiped — restart from scratch with: canfar agent setup")
...
def plugins_root(ctx: typer.Context) -> None:
    if ctx.invoked_subcommand is None:
        ...
        if opts.json:
            ui.print_json(
                {
                    "help": "canfar agent plugins --help",
        ...
        ui.print_hint("  canfar agent plugins list")
        ui.print_hint("  canfar agent plugins --help")
...
def plugins_install_cmd(
    ...
    Examples:
      canfar agent plugins install ray-manager-mcp
      canfar agent plugins install skore-cli ponytail-rule
    """
    opts = get_opts(ctx)
    names = list(plugins)
    errors: list[str] = []
