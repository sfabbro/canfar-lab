...
from canfar_lab.utils.json_utils import read_json
...
def relocate_agent_runtime_state(home: Path, *, dry_run: bool) -> list[str]:
    ...
    runtime goes to scratch via symlinks. See canfar_lab.core.home_layout.
    """
...
def agent_setup(
    *,
    ...
    def _run() -> SetupResult:
        ...
        if mode != "project":
            from canfar_lab.agent import review_bench as _review_bench
        ...
        if mode != "project":
            try:
                from canfar_lab.agent import review_bench as _rb_keys
        ...
        if not dry_run and mode != "project":
            if ok:
                ...
                from canfar_lab.agent.setup_state import lab_state_dir
