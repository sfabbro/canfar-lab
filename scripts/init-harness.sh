#!/usr/bin/env bash
# ponytail: one-shot bootstrap; upgrade path = copy template from agent-home
set -euo pipefail

ROOT="${1:-.}"
HARNESS="$ROOT/.cursor/harness"
REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
SKILL="${AGENT_HOME:-$HOME/.agent-home}/skills/harness-coding"
[[ -d "$SKILL" ]] || SKILL="$REPO_ROOT/skills/harness-coding"
